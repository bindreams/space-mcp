...
def make_account(name: str = "Anna Zhukova", username: str = "azhukova") -> SpaceAccount:
    ...
    return SpaceAccount(
        id=f"id-{username}", username=username, email=f"{username}@test.com", first_name=first, last_name=last
    )
...
def make_mr(**overrides) -> MergeRequest:
    defaults = dict(
        ...
        created_at=make_dt(),
        description=None,
        created_by=make_account(),
        participants=(
            Reviewer(user=make_account("John Doe", "jdoe"), role=ReviewRole.REVIEWER, state=ReviewState.PENDING),
        ),
...
def make_run(**overrides) -> PatronusRun:
    defaults = dict(
        ...
        branch_pair=BranchPair(
            source_branch="refs/patronus/safepush/abc", target_branch="master", repository="ultimate"
        ),
        owner=make_account(),
...
def make_check_config(name: str = "Compile All") -> PatronusCheckConfig:
    return PatronusCheckConfig(
        name=name,
        build_configuration_id=f"id_{name}",
        build_configuration_name=f"{name} Build",
        build_configuration_url=f"https://tc.example.com/{name}",
...
def make_check_run(name: str = "Compile All", status: RunStatus = RunStatus.SUCCESS, attempts=()) -> PatronusCheckRun:
    return PatronusCheckRun(
        id=f"check-{name}",
        config=make_check_config(name),
        status=status,
        queued_at=make_dt(hour=8),
        started_at=make_dt(hour=8, minute=1),
        finished_at=make_dt(hour=8, minute=5),
        skip_reason=None,
        attempts=attempts,
    )
