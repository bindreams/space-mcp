...
from space.context import GitContext, _parse_remote_url, detect_git_context, resolve_context, resolve_token, load_stored_token
...
class TestResolveToken:
    def test_env_var_takes_priority(self, monkeypatch):
        ...
        with patch("space.context.load_stored_token", return_value="stored-token"):
            assert resolve_token() == "env-token"

    @patch("space.context.load_stored_token", return_value="stored-token")
    def test_falls_back_to_stored_credential(self, mock_stored, monkeypatch):
        monkeypatch.delenv("SPACE_TOKEN", raising=False)
        assert resolve_token() == "stored-token"

    @patch("space.context.load_stored_token", return_value=None)
    def test_returns_none_when_nothing_available(self, mock_stored, monkeypatch):
        monkeypatch.delenv("SPACE_TOKEN", raising=False)
