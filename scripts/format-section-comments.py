...
# ///
"""Pre-commit hook: fix section comment formatting in Python files.
...
# Must be a plain # comment with text, NOT already a section comment.
...
def process_lines(lines: list[str]) -> tuple[list[str], bool]:
    ...
    for i, raw_line in enumerate(lines):
        ...
        if fill_m and new_lines:
            ...
            if (name_m and not SECTION_RE.match(prev_line) and name_m.group(2)):
        ...
        if i + 1 < len(lines):
            ...
            if fill_m2:
                ...
                if (name_m2 and not SECTION_RE.match(line) and name_m2.group(2)):
